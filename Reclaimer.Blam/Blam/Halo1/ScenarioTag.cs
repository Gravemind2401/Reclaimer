...
{
    public partial class ScenarioTag : ContentTagDefinition<Scene>
    {
        public ScenarioTag(IIndexItem item)
            : base(item)
        { }
        ...
        public override Scene GetContent()
        {
            ...
            foreach (var bspTag in ReadTags<ScenarioStructureBspTag>(StructureBsps.Select(b => b.BspReference)))
            {
                try
                {
            ...
            foreach (var skyTag in ReadTags<SkyTag>(Skies.EmptyIfNull().Select(b => b.SkyReference)))
            {
                try
                {
            ...
            foreach (var group in placements.GroupBy(x => x.PaletteIndex))
            {
                ...
                foreach (var placement in group)
                {
                    try
                    {
                        ...
                        sceneItem.SetTransform(1f, placement.Position.ToVector3(), (Quaternion)placement.Rotation);
    ...
    public partial class MachinePlacementBlock : PlacementBlockBase
    {

    }

    public partial class ControlPlacementBlock : PlacementBlockBase
    {
