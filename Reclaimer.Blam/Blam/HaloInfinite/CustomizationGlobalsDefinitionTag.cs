...
{
    public class CustomizationGlobalsDefinitionTag : ContentTagDefinition<Scene>, IContentProvider<Model>
    {
        [Offset(16)]
        public BlockCollection<ObjectTheme> Themes { get; set; }

        public CustomizationGlobalsDefinitionTag(ModuleItem item, MetadataHeader header)
            : base(item, header)
        { }
        ...
        private static void AddAttachment(TagReferenceGen5 attachmentReference, ModelRegion themeRegion, Model model)
        {
            ...
            if (attachment != null)
            {
                var attachments = attachment.ReadMetadata<CustomizationAttachmentConfigurationTag>().ModelAttachments.FirstOrDefault();
                if (attachments == null)
                    return;

                var attachmentModel = attachments.AttachmentModel.Tag?.ReadMetadata<ModelTag>().RenderModel.Tag?.ReadMetadata<RenderModelTag>().GetModelContent();
        ...
        public Model GetModelContent()
        {
            ...
            var armor = Themes[0].Model.Tag.ReadMetadata<RenderModelTag>().GetModelContent();
            ...
            foreach (var theme in Themes[0].ThemeConfigurations)
            {
                ...
                var themeConfig = theme.ThemeConfigs.Tag.ReadMetadata<CustomizationThemeConfigurationTag>();
