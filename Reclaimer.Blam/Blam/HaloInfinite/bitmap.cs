...
namespace Reclaimer.Blam.HaloInfinite
{
    public class bitmap : ContentTagDefinition<IBitmap>, IBitmap
    {
        ...
        int IBitmap.SubmapCount => Bitmaps.FirstOrDefault()?.BitmapType == TextureType.Array
                                    ? Bitmaps.FirstOrDefault()?.Depth ?? 0
                                    : Bitmaps?.Count ?? 0;

        CubemapLayout IBitmap.CubeLayout => CubemapLayout.NonCubemap;

        private record ResourceDimensions(ModuleItem Resource, int Height, int Width, bool HD1Available);
        ...
        private ResourceDimensions GetResourceAndDimensions(int index)
        {
            ...
            var dimensions = (height: submap.Height, width: submap.Width);
