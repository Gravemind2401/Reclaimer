...
function Rasterize-InstallerResources {
    ...
    Copy-Item ".\obj\Banner.png", ".\obj\Background.png" -Destination "$buildRoot\Reclaimer\Reclaimer.SetupPackage" -Force
}
...
function Build-Installer {
    ...
    dotnet build "$buildRoot\Reclaimer\Reclaimer.SetupPackage\Reclaimer.SetupPackage.wixproj" -c Release --property:AssemblyVersion=$assemblyVersion --property:DefineConstants="UseCustomImages" --no-dependencies
...
function Copy-Artifacts {
    ...
    # copy installer MSI
    Get-ChildItem "$buildRoot\Reclaimer\Reclaimer.SetupPackage\bin\Release" -Filter 'Reclaimer.SetupPackage.msi' -Recurse `
        | Copy-Item -Destination ".\bin\Reclaimer.Setup-v$assemblyVersion.msi"

    # cleanup python cache files
    Get-ChildItem "$buildRoot\Reclaimer\Reclaimer.RMFImporter\Reclaimer" -Directory -Recurse `
        | Where-Object { $_.Name -eq '__pycache__' } `
        | Remove-Item -Recurse -Force
    
    # create RMF Importer plugin zip
    Compress-Archive "$buildRoot\Reclaimer\Reclaimer.RMFImporter\Reclaimer" '.\bin\RMFImporter.zip' -CompressionLevel Optimal
}
...
# this requires inkscape to be installed and requires icomake to be in the resources folder
